		...
		<el-row :gutter="15">
			<el-col :lg="12">
				<el-card shadow="never" header="导入(使用mock,有50%几率导入失败)">
					<sc-file-import :apiObj="commonApi.importFile" templateUrl="http://www.scuiadmin/file.xlsx" @success="success"></sc-file-import>
					<sc-file-import :apiObj="commonApi.importFile" :data="{otherData:'demo'}" templateUrl="http://www.scuiadmin/file.xlsx" accept=".xls, .xlsx" :maxSize="30" tip="请上传小于或等于 30M 的 .xls, .xlsx 格式文件(自定义TIP)" @success="success">
			...
			<el-col :lg="12">
				<el-card shadow="never" header="导出">
					<sc-file-export :apiObj="commonApi.exportFile"></sc-file-export>
					<sc-file-export :apiObj="commonApi.exportFile" fileName="人员列表(异步)" async>
						<template #default="{open}">
							<el-button type="primary" icon="sc-icon-download" @click="open">导出(异步)</el-button>
						</template>
					</sc-file-export>
					<sc-file-export :apiObj="commonApi.exportFile" blob fileName="人员列表" :data="{otherData:'demo'}" showData :column="column" :fileTypes="['xlsx','docx','pdf']">
						<template #default="{open}">
...
<script>
	...
	import commonApi from '@/api/model/common.js'
